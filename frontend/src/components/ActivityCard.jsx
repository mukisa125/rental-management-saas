      ...
      <ul className="space-y-3">
        ...
        {activities.map((a, i) => (
          <li key={i} className="flex items-start gap-3 py-2 border-b last:border-b-0 border-slate-100">
            ...
            <div className={`text-xs ${a.type === 'success' ? 'text-green-600' : a.type === 'warning' ? 'text-amber-500' : 'text-slate-500'}`}>
              <span className="px-2 py-1 rounded-full bg-slate-50 text-xs">{a.badge}</span>
            </div>
          </li>
        ))}
      </ul>
